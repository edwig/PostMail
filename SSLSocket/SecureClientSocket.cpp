...
SecureClientSocket::InitializeSSL(const void* p_buffer, const int p_length)
{
  ...
  hr = SSPINegotiateLoop((TCHAR*) m_hostName.GetString());
	...
	if(FAILED(hr))
	{
		LogError(_T("Couldn't get stream sizes, hr=%#x"), hr);
    m_secureMode = false;
		return hr;
	}
...
SecureClientSocket::RecvPartial(LPVOID p_buffer, const ULONG p_length)
{
  ...
  {	
    // There are stored bytes, just return them
	  DebugMsg(_T("There is cached plaintext %d bytes"), m_plainTextBytes);
    if(false)
    {
      ...
      DebugMsg(_T("All %d cached plaintext bytes can be returned"), m_plainTextBytes);
      if(false)
      {
        PrintHexDump(m_plainTextBytes,m_plainTextPtr);
      }
      ...
      m_plainTextBytes -= p_length;
		  DebugMsg(_T("%d cached plaintext bytes can be returned, %d remain"), p_length, m_plainTextBytes);
      if(false)
      {
        PrintHexDump(m_plainTextBytes,m_plainTextPtr);
      }
  ...
  if(m_readBufferBytes == 0)
  {
    ...
    // There is already data in the buffer, so process it first
		DebugMsg(_T(" "));
		DebugMsg(_T("Using the saved %d bytes from server"), m_readBufferBytes);
    ...
    if (freeBytesAtEnd == 0) 
    {
      ...
      {
			  LogError(_T("RecvMsg Buffer inexplicably full"));
			  return SOCKET_ERROR;
      }
    }
    err = PlainSocket::RecvPartial((TCHAR*)m_readPointer + m_readBufferBytes, freeBytesAtEnd);
		m_lastError = 0; // Means use the one from m_SocketStream
		if ((err == SOCKET_ERROR) || (err == 0))
		{
      if(WSA_IO_PENDING == GetLastError())
      {
        LogError(_T("RecvMsg timed out"));
      }
      else if(WSAECONNRESET == GetLastError())
      {
        LogError(_T("RecvMsg failed, the socket was closed by the other host"));
      }
      else
      {
        LogError(_T("RecvMsg failed: %ld"),GetLastError());
      }
			return SOCKET_ERROR;
		}
		DebugMsg(_T(" "));
		DebugMsg(_T("Received %d bytes of ciphertext from server"), err);
  ...
  if(scRet == SEC_E_OK)
  {
    DebugMsg(_T("Decrypted message from server."));
  }
	else if(scRet == SEC_I_CONTEXT_EXPIRED)
	{
    DebugMsg(_T("Server signaled end of session"));
		m_lastError = scRet;
		return SOCKET_ERROR;
	}
	else
	{
		LogError(_T("Couldn't decrypt data from server, error %lx"), scRet);
		m_lastError = scRet;
		return SOCKET_ERROR;
	}
  ...
  if(p_length >= int(pDataBuffer->cbBuffer))
  {
    ...
    // More bytes were decoded than the caller requested, so return some, store the rest until the next call
      ...
      m_plainTextPtr = m_plainText;
   	DebugMsg(_T("Extra %d plaintext bytes stored"), m_plainTextBytes);
		if (memcpy_s(m_plainText, sizeof(m_plainText), (TCHAR*)pDataBuffer->pvBuffer + p_length, m_plainTextBytes))
		{
	...
	{	// More data was read than is needed, this happens sometimes with TCP
		DebugMsg(_T(" "));
		DebugMsg(_T("Some extra ciphertext was read (%d bytes)"), pExtraDataBuffer->cbBuffer);
	...
	{
		DebugMsg(_T("No extra ciphertext was read"));
		m_readBufferBytes = 0;
		m_readPointer = m_readBuffer;
	}
...
int SecureClientSocket::SendPartial(LPCVOID p_buffer, const ULONG p_length)
{
  ...
  INT err;
	...
	DebugMsg(_T("SendPartial %d encrypted bytes to server"), Buffers[0].cbBuffer + Buffers[1].cbBuffer + Buffers[2].cbBuffer);
	PrintHexDump(Buffers[0].cbBuffer + Buffers[1].cbBuffer + Buffers[2].cbBuffer, m_writeBuffer);
	
  if (err == SOCKET_ERROR)
	{
		LogError( _T("SendPartial failed: %ld"), GetLastError());
		return SOCKET_ERROR;
	}
	return p_length;
} // SendPartial
...
SecureClientSocket::SSPINegotiateLoop(TCHAR* ServerName)
{
  ...
  {
		LogError(_T("**** Error %#x returned by InitializeSecurityContext (1)"), scRet);
    return scRet;
  }
  ...
  if(OutBuffers[0].cbBuffer != 0 && OutBuffers[0].pvBuffer != NULL)
  {
    ...
    {
      LogError(_T("**** Error %d sending data to server (1)"), WSAGetLastError());
      g_pSSPI->FreeContextBuffer(OutBuffers[0].pvBuffer);
		  m_context.Close();
      return SEC_E_INTERNAL_ERROR;
    }

    DebugMsg(_T("%d bytes of handshake data sent"), cbData);
        ...
        scRet == SEC_I_INCOMPLETE_CREDENTIALS) 
  {
    ...
    if(0 == cbIoBuffer || scRet == SEC_E_INCOMPLETE_MESSAGE)
    {
      if(fDoRead)
      {
        ...
        {
          LogError(_T("**** Error %d reading data from server"), WSAGetLastError());
          scRet = SEC_E_INTERNAL_ERROR;
          break;
        }
        else if(cbData == 0)
        {
          LogError(_T("**** Server unexpectedly disconnected"));
          scRet = SEC_E_INTERNAL_ERROR;
          break;
        }

        DebugMsg(_T("%d bytes of handshake data received"), cbData);
       ...
       FAILED(scRet) && (dwSSPIOutFlags & ISC_RET_EXTENDED_ERROR))
    {
      ...
      if (FAILED(hr))
      {
        ...
        {
          DebugMsg(_T("QueryContextAttributes for cert returned SEC_E_INVALID_HANDLE, which is normal"));
        }
        else
        {
          LogError(_T("Couldn't get server certificate, hr=%#x"),hr);
        }
      }
      else
      {
        DebugMsg(_T("Server Certificate returned"));
      ...
      if (OutBuffers[0].cbBuffer != 0 && OutBuffers[0].pvBuffer != NULL)
      {
        ...
        if(cbData == SOCKET_ERROR || cbData == 0)
        {
          ...
          {
            LogError(_T("**** Server closed the connection unexpectedly"));
          }
          else
          {
            LogError(_T("**** Error %d sending data to server (2)"),err);
          }
          g_pSSPI->FreeContextBuffer(OutBuffers[0].pvBuffer);
          m_context.Close();
          return SEC_E_INTERNAL_ERROR;
    ...
    if (scRet == SEC_E_OK)
    {
      ...
      DebugMsg(_T("Handshake was successful"));
      ...
      if(InBuffers[1].BufferType == SECBUFFER_EXTRA)
      {
        ...
        DebugMsg(_T("%d bytes of app data was bundled with handshake data"), m_readBufferBytes);
      }
      else
      {
        m_readBufferBytes = 0;
    ...
    if(FAILED(scRet))
    {
      LogError(_T("**** Error %#x returned by InitializeSecurityContext (2)"),scRet);
      break;
    }
    ...
    if(scRet == SEC_I_INCOMPLETE_CREDENTIALS)
    {
      ...
      {
        LogError(_T("**** Error %08x returned by GetNewClientCredentials"), scRet);
        break;
      }
    }
...
SecureClientSocket::Disconnect(int p_side /*= SD_BOTH*/)
{
  ...
  if(pbMessage != NULL && cbMessage != 0)
  {
    ...
    DebugMsg(_T("Sending Close Notify"));
    DebugMsg(_T("%d bytes of handshake data sent"), cbData);
...
SecureClientSocket::GetNewClientCredentials()
{
  ...
  if(Status != SEC_E_OK)
  {
    LogError(_T("Error 0x%08x querying issuer list info"), Status);
    return Status;
  }

  DebugMsg(_T("Issuer list information returned, issuers = %d"), IssuerListInfo.cIssuers);
  ...
  if(FAILED(Status))
  {
    LogError(_T("Error 0x%08x selecting client certificate"), Status);
    return Status;
  }
  attach(hCertContext, pCertContext);
  if(!hCertContext)
  {
    LogError(_T("No suitable client certificate is available to return to the server"));
  }
...
SecureClientSocket::CreateCredentialsFromCertificate(PCredHandle phCreds, PCCERT_CONTEXT pCertContext)
{
  ...
  if (Status != SEC_E_OK)
  {
    ...
    {
      LogError(_T("**** Error: 'Unknown Credentials' returned by AcquireCredentialsHandle. LastError=%d"),dw);
    }
    else
    {
...
SecureClientSocket::SendMsg(LPCVOID p_buffer,const ULONG p_length)
{
  ...
  while(total_bytes_sent < p_length)
  {
    ...
    bytes_sent = SecureClientSocket::SendPartial((BYTE*)p_buffer + total_bytes_sent,toSend);
...
SecureClientSocket::RecvMsg(LPVOID p_buffer,const ULONG p_length)
{
  ...
  {
    bytes_received = SecureClientSocket::RecvPartial((BYTE*)p_buffer + total_bytes_received,p_length - total_bytes_received);
