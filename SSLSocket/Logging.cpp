...
void LogError(const TCHAR* p_format,...)
{
  ...
  {
    TCHAR buf[bufferLength];
    StringCchPrintf(buf,sizeof(buf) / sizeof(TCHAR),_T("(%lu) : ERROR : "),GetCurrentThreadId());

    va_list arglist;
    va_start(arglist,p_format);
    StringCchVPrintf(&buf[_tcslen(buf)],sizeof(buf) / sizeof(TCHAR),p_format,arglist);
    va_end(arglist);

    StringCchCat(buf,sizeof(buf) / sizeof(TCHAR),_T("\n"));
    (*printing)(buf);
  }
}
...
void DebugMsg(const TCHAR* p_format, ...)
{
  ...
  {
    TCHAR buf[bufferLength];
    StringCchPrintf(buf, sizeof(buf)/sizeof(TCHAR), _T("(%lu) : "), GetCurrentThreadId());
...
static void PrintHexDumpActual(DWORD p_length,const void* p_buffer)
{
  DWORD count = 0;
	...
	TCHAR line[1024];
	int   pos = 0;
	BYTE* buffer = static_cast<BYTE*>(const_cast<void*>(p_buffer));
  ...
  if((SSL_socket_logging < SOCK_LOGGING_FULLTRACE) && (p_length > 16))
  {
    ...
    DWORD i = 0;
		...
		_stprintf_s(line, 8, _T("%4.4x  "), index);
		pos = 6;
		...
		for(i = 0; i < count; i++) 
		{
      if(buffer[i] < 32 || buffer[i] > 126 || buffer[i] == _T('%'))
      {
        line[pos++] = _T('.');
      }
      else
