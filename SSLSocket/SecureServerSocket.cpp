...
int SecureServerSocket::RecvPartial(LPVOID p_buffer,const ULONG p_length)
{
  ...
  if(m_readBufferBytes == 0)
  {
	...
	{
		err = PlainSocket::RecvPartial((TCHAR*)m_readPointer + m_readBufferBytes,(int)( sizeof(m_readBuffer) - m_readBufferBytes - ((TCHAR*)m_readPointer - &m_readBuffer[0])));
    m_lastError = 0; // Means use the one from m_SocketStream
      ...
      {
        LogError(_T("Recv failed, the socket was closed by the other side"));
      }
      else
      {
        LogError(_T("Recv failed: %ld"),GetLastError());
      }
			return SOCKET_ERROR;
		}
		DebugMsg(_T(" "));
...
int SecureServerSocket::SendPartial(LPCVOID p_buffer, const ULONG p_length)
{
  ...
  {
    LogError(_T("Send failed: No buffer or size too great"));
    return SOCKET_ERROR;
  }
...
SecureServerSocket::SendMsg(LPCVOID p_buffer,const ULONG p_length)
{
  ...
  {
    bytes_sent = SecureServerSocket::SendPartial((BYTE*)p_buffer + total_bytes_sent,p_length - total_bytes_sent);
