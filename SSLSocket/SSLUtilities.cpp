...
void SetSSLThreadName(TCHAR* threadName)
{
  SetSSLThreadName(threadName,MAXDWORD);
}

void SetSSLThreadName(TCHAR* threadName,DWORD dwThreadID)
{
...
HRESULT ShowCertInfo(PCCERT_CONTEXT pCertContext, CString Title)
{
  ...
  else
  {
	...
	{
    CString extension = A2T((p++)->pszObjId);
		DebugMsg(_T("Extension %s"),extension.GetString() );
	}
    ...
    switch(dwPropId)
    {
      ...
      default:                                      DebugMsg(_T("Unknown property"));                         break;
    } // End switch.
...
SECURITY_STATUS CertFindServerByName(PCCERT_CONTEXT & pCertContext, LPCTSTR pszSubjectName, boolean fUserStore)
{
  ...
  if (!pCertContext)
  {
    ...
    if (LastError == CRYPT_E_NOT_FOUND)
    {
      DebugMsg(_T("**** CertFindCertificateInStore did not find a certificate, creating one"));
      pCertContext = CreateCertificate(true, pszSubjectName);
      if (!pCertContext)
      {
        LastError = GetLastError();
        LogError(_T("**** Error 0x%x returned by CreateCertificate"), LastError);
        LogError(_T("Could not create certificate, are you running as administrator?"));
        return HRESULT_FROM_WIN32(LastError);
      }
    }
    else
