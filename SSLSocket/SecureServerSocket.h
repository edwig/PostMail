...
private:
  ...
  // Private data
	...
	static const int  m_maxExtraSize =   384;  // Also arbitrary, current header is 5 bytes, trailer 36 => VALUES 41 / 50 / 384
	TCHAR              m_writeBuffer[m_maxMsgSize + m_maxExtraSize];       // Enough for a whole encrypted message
	TCHAR              m_readBuffer[(m_maxMsgSize + m_maxExtraSize) * 2];  // Enough for two whole messages so we don't need to move data around in buffers
